...
| **2 Pull/push comp** | Width-graduated `clamp(0.1+0.12·w, 0.2..0.7)` + fabric `scale`; push = end-trim on open columns (`satin.ts:148,162`); plus physical inverse `precompensate` (`bench/distortion.ts`) **not default-on** | **partial** | No percentage-of-width mode; min-sewable-width floor now enforced (`satin.ts` MIN_SEWABLE_SATIN_WIDTH=1.0, per-throw widen) with a user-facing narrow-column warning in validate.ts; fill pull comp / edge overlap not explicit; circle-axis comp not modeled; physical model parameterized + per-fabric calibration loop shipped (lib/calibration.ts); off unless the project carries a fitted calibration |
