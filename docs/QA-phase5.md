...
Resizing scales the **millimeter geometry**, never stitch points. Stitches are
...
- `layout`: design bounds/size, uniform resize-to-width, fit-to-hoop (clamped +
  centered), and the re-densification acceptance test.
- `worksheet`: color-stop grouping with per-stop counts, duration formatting,
  and self-contained HTML generation (swatches, brand, totals).
- `DesignPanel` (jsdom): resize commits on blur, fit-to-hoop centers, hoop preset
  switching.
...
| PES colors look slightly off | Brother palette snapping in #PES0001 | #PES0060 (truer color) is selectable in the export menu. |

## Known limitations (tracked)

- Estimated run time uses a fixed 600 spm + 20 s/color-change; real machines
  vary — it's a ballpark, clearly labeled "Est."
