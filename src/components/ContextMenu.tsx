...
/** Menu width (px) — matches the `w-52` Tailwind class. */
const MENU_W = 208;

/**
 * Clamp the menu's top-left so the whole menu stays on-screen near every edge.
 * On touch the rows are finger-sized, so a 20-action menu is tall — we estimate
 * its height from the row count (coarse pointer ⇒ taller rows) and pin it inside
 * the viewport. Pure so the on-screen guarantee is unit-tested without a browser.
 */
