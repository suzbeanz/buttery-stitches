...
export function buildTemplate(id: string, font: Font, words?: string): Project {
  ...
  if (id === "name-patch") {
    ...
    bar.name = "Border";
    const t = layoutText({ text, font, heightMm: 12, colorId: cream.id, name: text });
    ...
    band.name = "Band";
    const t = layoutText({ text: text.slice(0, 1), font, heightMm: 26, colorId: cream.id, name: "Letter" });
    const letter = place(t.object, cx, cy);
    letter.name = text.slice(0, 1);
    p.objects.push(band, letter);
  } else if (id === "team-arc") {
    const arcTop = layoutText({ text, font, heightMm: 12, archDeg: 90, colorId: navy, name: text });
    const top = place(arcTop.object, cx, cy - 10);
    top.name = text;
    const est = layoutText({ text: "EST. 2026", font, heightMm: 5, colorId: navy, name: "EST" });
